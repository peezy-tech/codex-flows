...
describe("memory transplant", () => {
	test("dry-run reports adds, conflicts, skipped files, and bytes", async () => {
		const { globalHome, workspaceHome, workspaceRoot } = await memoryHomes();
		...
		await writeFile(path.join(workspaceHome, "memories", "memory_summary.md"), "workspace\n");
		const plan = await planMemoryTransplant({
			direction: "global-to-workspace",
			workspaceRoot,
			globalCodexHome: globalHome,
			workspaceCodexHome: workspaceHome,
		});
	...
	test("copies only durable artifacts from a real Codex memory layout", async () => {
		...
		await writeFile(path.join(globalHome, "memories", "phase2_workspace_diff.md"), "diff\n");
		await writeFile(path.join(globalHome, "memories", "state_5.sqlite"), "db");
		...
		expect(await readFile(path.join(workspaceHome, "memories", "raw_memories.md"), "utf8")).toBe("raw\n");
		expect(await exists(path.join(workspaceHome, "memories", ".git", "HEAD"))).toBe(false);
		expect(await exists(path.join(workspaceHome, "memories", "extensions", "ad_hoc", "instructions.md"))).toBe(false);
	});
	...
	test("apply copies missing files and leaves conflicts without overwrite", async () => {
		...
		await writeFile(path.join(globalHome, "memories", "raw_memories.md"), "raw\n");
		await writeFile(path.join(workspaceHome, "memories", "MEMORY.md"), "workspace\n");
		...
		expect(await readFile(path.join(workspaceHome, "memories", "raw_memories.md"), "utf8")).toBe("raw\n");
		expect(await readFile(path.join(workspaceHome, "memories", "MEMORY.md"), "utf8")).toBe("workspace\n");
	});
...
async function memoryHomes(): Promise<{
	...
	const workspaceHome = path.join(workspaceRoot, ".codex");
	for (const dir of [
		path.join(globalHome, "memories", "rollout_summaries"),
		path.join(globalHome, "memories", ".git"),
		path.join(workspaceHome, "memories", "rollout_summaries"),
	]) {
		await mkdir(dir, { recursive: true });
	}
	return { workspaceRoot, globalHome, workspaceHome };
}
