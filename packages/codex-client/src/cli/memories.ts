...
export async function planMemoryTransplant(
	...
	const workspaceHome = path.resolve(options.workspaceCodexHome ?? path.join(workspaceRoot, ".codex"));
	const sourceHome = options.direction === "global-to-workspace" ? globalHome : workspaceHome;
	const destinationHome = options.direction === "global-to-workspace" ? workspaceHome : globalHome;
